        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            try {
                ...
                if (!access_token1.equals(access_token)) {
                    throw ClientException.of(HttpStatus.UNAUTHORIZED, "It's not current refresh token");
                }
                ...
                response.setContentType(MediaType.APPLICATION_JSON_VALUE);
                try {
                    new ObjectMapper().writeValue(response.getOutputStream(), new ClientException.Response(e.getCode(), e.getMessage()));
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            try {
                ...
                if (!oldRefreshToken.equals(refresh_token)) {
                    throw ClientException.of(HttpStatus.FORBIDDEN, "It's not current refresh token");
                }
